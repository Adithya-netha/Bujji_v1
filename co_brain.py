...
from Brain.brain import get_chat_completion
...
def check_inputs():
    ...
    while True:
        ...
        if input_text != output_text:
            ...
            if output_text.startswith("tell me"):
            ...
            elif "bujji" in output_text:
                f = open('log.txt','a')
                f.write('\n'+'You : '+ output_text)
                response = get_chat_completion(output_text)
                f.write('\n'+'jarvis : '+ response)
                speak(response)
            ...
            elif "check weather" in output_text or "weather" in output_text:
                text = output_text.replace("check weather in","")
                text = output_text.replace("weather in","")
               ...
               volume_level_str = input_text.split("to")[-1].strip().rstrip("%")  # Get the part after "to", remove spaces and "%"
               try:
                  volume_level = int(volume_level_str)  # Convert to integer
                  set_volume_windows(volume_level)  # Pass to your function
               except ValueError:
                 print("Error: Invalid volume level provided.")
